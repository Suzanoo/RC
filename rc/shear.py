...
class ShearReinforcement:
    ...
    def beamTraverse(self, b, d, Av, Vu):
        ...
        lg = (
            (1 / 3) * self.𝜙v * np.sqrt(self.fc) * b * d * 1e-1
        )  # light shear, kN

        hv = (
            (2 / 3) * self.𝜙v * np.sqrt(self.fc) * b * d * 1e-1
        )  # heavy shear, kN

        if Vu <= 𝜙Vc:
            s_max = min(
                3 * Av * self.fv / b, d / 2, 60
            )  # cm, ACI 11.5.5.3;11-13
            print("case1")
    ...
    def deepBeam(self, b, d, ln):
        ...
        while True:
            ...
            Avhmin = 0.0025 * b * s2
            print(f"Avh min = {Avhmin:.2f} cm2, please select diameter of horizontal rebar.")
        ...
        # Shear capcity 
        𝜙Vs = self.𝜙v * ((Av / s) * self.fv * d * ((1 + (ln / d))/12) + (Avh / s2) * self.fy * d * ((11 - ln / d)/12)) * 1e-1 # kN
        return 𝜙Vs
