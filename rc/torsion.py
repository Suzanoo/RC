...
class Torsion:
    ...
    def traverse(self, bw, a=45):
        ...
        self.At_ratio = ((𝜙Tn * np.tan(a) * 1e6) / (self.𝜙v * 2 * Ao * self.fyv)) # mm2/mm
    ...
    def longitudinal_reinf(self, bw, Acp, a=45):
        ...
        Al = (self.At_ratio * Ph  * (self.fyv / self.fyt) * (1 / (np.tan(a) ** 2))) * 1e-2 # cm2

        Al_ratio = self.At_ratio if (self.At_ratio > (bw / (6 * self.fyv))) else bw / (6 * self.fyv)

        Almin = (((5 * np.sqrt(self.fc) * Acp) / (12 * self.fyt)) - Al_ratio * Ph * (self.fyv / self.fyt)) * 1e-2 # cm2
